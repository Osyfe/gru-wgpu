pub use gru_ui::{math::Vec2, event::{HardwareEvent, MouseButton, Key}};
use winit::{window::{Window, CursorGrabMode}, event::{DeviceEvent, WindowEvent, ElementState, MouseButton as WinitMouseButton, MouseScrollDelta}, keyboard::{PhysicalKey, KeyCode}, dpi::PhysicalPosition};
...
impl Input
{
    ...
    {
        Self { cam_mode: false, pointer_pos: Vec2(0.0, 0.0), events: Vec::new() }
    }

    pub(crate) fn event(&mut self, raw_event: RawEvent)
    {
        match raw_event
        {
            RawEvent::Device(event) => match event
            {
                DeviceEvent::MouseMotion { delta } => if self.cam_mode
                {
            ...
            {
                WindowEvent::CloseRequested => self.events.push(HardwareEvent::CloseWindow),
                WindowEvent::CursorMoved { position, .. } => if !self.cam_mode
                {
                    let new_pos = Vec2(position.x as f32, position.y as f32);
                    let delta = new_pos - self.pointer_pos;
                    self.pointer_pos = new_pos;
                    let event = HardwareEvent::PointerMoved { pos: self.pointer_pos, delta };
                    self.events.push(event);
                },
                WindowEvent::MouseInput { state, button, .. } =>
                {
                    ...
                    let event = HardwareEvent::PointerClicked { pos: self.pointer_pos, button, pressed: state == ElementState::Pressed };
                    self.events.push(event);
                },
                WindowEvent::CursorLeft { .. } => self.events.push(HardwareEvent::PointerGone),
                WindowEvent::MouseWheel { delta: MouseScrollDelta::LineDelta(dx, dy), .. } => self.events.push(HardwareEvent::Scroll { pos: self.pointer_pos, delta: Vec2(dx, dy) }),
